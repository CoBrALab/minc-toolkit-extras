...
if ! [[  ( $firstarg = *csv ) ||  ( $firstarg = *mnc )  ]]; then
    echo "usage: $0 labels.mnc jacobian1.mnc [ jacobian2.mnc ... jacobianN.mnc ]"
    exit 1
fi
...
for file in "$@"
do
    sem -j+0 "nii2mnc -quiet ${file} ${tmpdir}/$(basename ${file} .nii.gz).mnc && label_volumes_from_jacobians ${labels} ${tmpdir}/$(basename ${file} .nii.gz).mnc | tail -n +2 | cut -f2 -d"," | { echo -n "$file,"; awk -f<(echo '$AWKCMD') ; } | tr -d '[:space:]'"
done
sem --wait
