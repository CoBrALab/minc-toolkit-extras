...
for transform in transforms:
  if (slicestart + slicesize >= len(shrinks) - 1):
    print(transform, end=' \\\n')
    print("\t--metric Mattes[${{target}},${{atlas}},1,{},Regular,0.5]".format(bins[-1]), end=' \\\n')
    ...
    print(transform, end=' \\\n')
    print("\t--metric Mattes[${{target}},${{atlas}},1,{},Regular,0.5]".format(bins[slicestart+slicesize]), end=' \\\n')
...
print("\t--convergence [{},1e-9,20]".format("x".join(iterations[len(shrinks)-5:])), end=' \\\n')
