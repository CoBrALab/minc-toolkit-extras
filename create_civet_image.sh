...
cut -d " " -f $leftcolumn $leftstatmap | paste -d\* /opt/quarantine/resources/CIVET-CC-mask.txt - | sed 's/[eE]+\{0,1\}/*10^/g' | bc > $TMPDIR/$(basename $leftstatmap)
cut -d " " -f $rightcolumn $rightstatmap | paste -d\* /opt/quarantine/resources/CIVET-CC-mask.txt - | sed 's/[eE]+\{0,1\}/*10^/g' | bc > $TMPDIR/$(basename $rightstatmap)
